...
class PermainanSederhana:
    ...
    def reset(self):
        ...
        self.poin = 0
        self.permainan_selesai = False
        return self.posisi_pemain, self.poin
    ...
    def langkah(self, aksi):
        if aksi == 'kiri':
            ...
            print("Aksi tidak valid! Aksi harus 'kiri' atau 'kanan'.")
            return self.posisi_pemain, self.poin, self.permainan_selesai
        
        self.posisi_pemain = max(0, min(self.posisi_pemain, self.panjang_papan - 1))
        
        if self.posisi_pemain == self.posisi_lubang:
            self.poin += self.hadiah['lubang']
            self.posisi_pemain = self.posisi_awal
        elif self.posisi_pemain == self.posisi_apel:
            self.poin += self.hadiah['apel']
            self.posisi_pemain = self.posisi_awal
        else:
            self.poin += self.hadiah['langkah']
        
        if self.poin >= self.poin_menang:
            self.permainan_selesai = True
            print("Pemain menang!")
        elif self.poin <= self.poin_kalah:
            self.permainan_selesai = True
            print("Pemain kalah!")
